<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=00bfbf&height=120&section=header"/>

<h1 style="font-family: sans-serif;">Hello guys, welcome to my profile, my name is Padua Feijão!</h1>

I am a software developer specializing in frontend, I have extensive experience in creating web, mobile and rest API systems.

My contact means below

[![Whatsapp Badge](https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](https://api.whatsapp.com/send?phone=5588992810879)
[![Linkedin Badge](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/antonio-de-p%C3%A1dua-alves-feij%C3%A3o-43054b192)


<h1 style="font-family: sans-serif;">Frontend Skills</h1>
<ul>
  <li class="text">ReactJS</li>
  <li class="text">NextJS</li>
  <li class="text">Typescript</li>
  <li class="text">Javascript</li>
  <li class="text">HTML</li>
  <li class="text">CSS</li>
  <li class="text">TailwindCSS</li>
  <li class="text">Styled-Components</li>
  <li class="text">Bootstrap</li>
  <li class="text">Redux</li>
  <li class="text">Redux Sagas</li>
  <li class="text">Zustand</li>
  <li class="text">Axios</li>
  <li class="text">React Testing Library</li>
  <li class="text">Cypress</li>
</ul>

<h1 style="font-family: sans-serif;">Mobile Skills</h1>
<ul>
  <li class="text">React Native</li>
  <li class="text">Expo</li>
  <li class="text">Flutter</li>
  <li class="text">Dart</li>
  <li class="text">Native Base</li>
</ul>

<h1 style="font-family: sans-serif;">Backend Skills</h1>
<ul>
  <li class="text">NodeJS</li>
  <li class="text">Express.JS</li>
  <li class="text">PostgreSQL</li>
  <li class="text">MySQL</li>
  <li class="text">SQLite</li>
  <li class="text">MongoDB</li>
  <li class="text">Redis</li>
  <li class="text">Firebase</li>
  <li class="text">Jest</li>
</ul>


<h1 style="font-family: sans-serif;">Other Skills</h1>
<ul>
  <li class="text">Docker</li>
  <li class="text">AWS</li>
  <li class="text">Google Cloud</li>
  <li class="text">Git</li>
  <li class="text">Microservices</li>
</ul>
<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=00bfbf&height=120&section=footer"/>
